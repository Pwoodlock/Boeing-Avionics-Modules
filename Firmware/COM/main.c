...
unsigned long standbyMSB, standbyLSB, directionMSB, directionLSB;
	...
	void YourHighPriorityISRCode()
	{
		    ...
		    {                                   //check for TMR0 overflow
				...
				if(ENC_A != old_ENC_A)
				{
						...
						{
							display2Value-=250;
							if(display2Value<118000)display2Value=136750;
						}
						else
						{	
							display2Value+=250;
							if(display2Value>136750)display2Value=118000;
						}
					
					standbyMSB = display2Value/1000;
					standbyLSB = display2Value-(standbyMSB*1000);
				}
				...
				if(ENC_C != old_ENC_C)
				{
						...
						{
							standbyMSB--;
							if(standbyMSB==117)standbyMSB=136;
						}
						else
						{
							standbyMSB++;
							if(standbyMSB==137)standbyMSB=118;
						}
	
						display2Value = (standbyMSB*1000)+standbyLSB;
				}
				...
				else
				{
				  ...
				  PORTA = (numbers[ucDisplay1[5]] & 0x3F);
				  PORTE = ((numbers[ucDisplay1[5]]>>6) & 0x03);
				  Delay100TCYx(50);
				  ...
				  Delay100TCYx(50);
				  //Display Character 12 = ON
				  PORTD = 0;
				  PORTB = 0x08;
				  //Display Character 12 data
				  PORTA = (numbers[ucDisplay2[5]] & 0x3F);
				  PORTE = ((numbers[ucDisplay2[5]]>>6) & 0x03);
				  //Delay100TCYx(50);
...
{   
    ...
    {   
        ...
        switch(ReceivedDataBuffer[0])
        {
            ...
            case 0x82:  //Request for TFR button press status
				if(pb1Pressed == 1)
				{	
			...
			case 0x72:  //Request for TFR button press status
				if(pb2Pressed == 1)
				{	
					ToSendDataBuffer[0] = 0x72;				
					//load the display data into the send buffer
					ToSendDataBuffer[1] = 0x01;				//first byte says TFR button has been pressed		
					pb2Pressed=0;
				}
...
void display1(unsigned long value)
{
	...
	if(value>999999)value=999999;	//should never do this
