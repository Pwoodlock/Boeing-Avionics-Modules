...
signed long display1Value, display2Value, displaySwapValue;
signed long standbyMSB, standbyLSB, directionMSB, directionLSB;
BYTE old_ENC_A,old_ENC_C;
BYTE old_SW1, old_SW2;
BYTE sw1Changed, sw2Changed;
	...
	void YourHighPriorityISRCode()
	{
			...
			if(ENC_A != old_ENC_A)
			{
				updateEncoders();
					...
					{
						display2Value--;
						if(display2Value<0)display2Value=17999;
					}
					else
					{	
						display2Value++;
						if(display2Value>17999)display2Value=0;
					}
				
				standbyMSB = display2Value/10;
				standbyLSB = display2Value-(standbyMSB*10);
			}
			...
			if(ENC_C != old_ENC_C)
			{
				updateEncoders();
					...
					{
						standbyMSB--;
						if(standbyMSB<0)standbyMSB=1799;
					}
					else
					{
						standbyMSB++;
						if(standbyMSB==1800)standbyMSB=0;
					}

				display2Value = (standbyMSB*10)+standbyLSB;
			...
			display2(display2Value);
			  ...
			  PORTE = ((numbers[ucDisplay1[2]]>>6) & 0x03);	
			  Delay100TCYx(50);
			  ...
			  PORTE = ((numbers[ucDisplay1[3]]>>6) | 0x01);	//decimal point on
			  Delay100TCYx(50);
			  ...
			  Delay100TCYx(50);
			  //Display Character 6 = ON
			  PORTD = 0x40;
			  PORTB = 0;
			  //Display Character 6 data
			  ...
			  PORTE = ((numbers[ucDisplay2[2]]>>6) & 0x03);
			  Delay100TCYx(50);
			  ...
			  PORTE = ((numbers[ucDisplay2[3]]>>6) | 0x01);	//decimal point on
			  Delay100TCYx(50);
			  ...
			  //only do this if sw2 has NOT already been pressed, otherwise, the displays will keep swapping while the button is held
			  if(!pb1Pressed)
			  {	
				  if(pb1==0)		// if sw2 is pressed
			      {
		  		  	Timeout=0x1FF;	//reset timeout value
...
{   
    ...
    {   
        ...
        switch(ReceivedDataBuffer[0])
        {
            ...
            case 0x81:  //Request for TFR button press status
				if(pb1Pressed == 1)
				{	
					...
					ToSendDataBuffer[2] = ucDisplay1[0];	
					ToSendDataBuffer[3] = ucDisplay1[1];		
					ToSendDataBuffer[4] = ucDisplay1[2];		
					ToSendDataBuffer[5] = ucDisplay1[3];		
					ToSendDataBuffer[6] = ucDisplay1[4];			
					pb1Pressed=0;
				}
				else
				{
