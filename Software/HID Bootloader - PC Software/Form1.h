/*********************************************************************
 *
 *                Microchip USB HID Bootloader Version 1.0
 *
 *********************************************************************
 * FileName:        Form1.h
	...
	*/
	...
	private: System::Windows::Forms::Timer^  tmr_ThreadStatus;
		...
		void InitializeComponent(void)
		{
			...
			this->Name = L"Form1";
			this->Text = L"FDSI Bootloader v1.0";
		...
		private: void QueryThreadStart()
		{
			...
			if(ErrorStatus == ERROR_SUCCESS)
			{
				...
				if(ErrorStatus == ERROR_SUCCESS)
				{
					...
					bytesPerAddress = myResponse.QueryResults.BytesPerAddress;
					...
					if(HasOverlappedIoCompleted(&OverlappedReadStructure))
					{
						...
						for(i=0;i<myResponse.GetDataResults.BytesPerPacket;i++)
						{
							...
							if(tempData!=tempData2)
							{
								...
								#if !defined(ENCRYPTED_BOOTLOADER)
								else if(myResponse.GetDataResults.Address == PIC24_RESET_REMAP_OFFSET)
								{
									unsigned char k;
								...
								else if(myResponse.GetDataResults.Address = 0x00)
								{
									switch(i)
									{
										case 0:
